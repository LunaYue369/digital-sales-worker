...
Directory layout:
  ...
  config/{dir}/              — per-user config (gmail_token, copywriters/)
...
def _get_user_dir(user_id: str) -> str | None:
    """Return the dir name for a user (e.g. 'demo_user'), or None if not registered."""
...
def list_templates(user_id: str) -> list[str]:
    """Return the list of template names configured for a user from users.json.

    Returns whatever is in config['templates'] keys — no auto-add of 'default'.
    A user with no templates dict (e.g. admins) gets an empty list.
    """
    config = get_user_config(user_id)
    if not config:
        return []
