...
from core.user_config import get_drive_folder_id, get_template_config, list_templates
...
def run_auto_pipeline(user_id: str, say, client=None, channel: str = ""):
    folder_id = get_drive_folder_id(user_id)
    say(f":file_folder: Auto run started. Scanning Drive folder `{folder_id or 'NOT SET'}`...")

    try:
        found = _poll_once(user_id, say, client, channel)
        if not found:
            say(":hourglass_flowing_sand: No unprocessed files in Drive — run `prospect` first to generate leads.")
    except Exception as e:
        log.error("Pipeline error: %s", e, exc_info=True)
        say(f":x: Pipeline error: {e}")

    # If the running flag was cleared mid-run, the user pressed `stop auto`.
    interrupted = not state.is_auto_running(user_id)
    state.stop_auto(user_id)

    if interrupted:
        say(":stop_sign: Auto run interrupted by `stop auto`.")
    else:
        say(":white_check_mark: Auto run complete.")


def _poll_once(user_id: str, say, client=None, channel: str = "") -> bool:
    ...
    for file_info in new_files:
        ...
        try:
            _process_file(user_id, file_info, say, client, channel)
            drive_poller.mark_processed(user_id, file_id, file_name)
        except Exception as e:
...
def _extract_template_from_filename(name: str) -> str | None:
    """Extract template tag from `<anything>__<template>.(csv|xlsx|xls)` filenames.

    Template tag may contain letters, digits, and hyphens — no underscores,
    so the last `__` in the name cleanly separates body from tag.
    """
...
def _process_file(user_id: str, file_info: dict, say, client=None, channel: str = ""):
    campaign_id = f"campaign_{uuid.uuid4().hex[:8]}"

    # Template is chosen by the CSV filename suffix `__<template>.csv`.
    # No suffix or unknown template → skip the file (do not silently fall back).
    filename_template = _extract_template_from_filename(file_info["name"])
    if not filename_template:
        say(f":warning: `{file_info['name']}` has no `__<template>` suffix — skipping. "
            f"Re-run `prospect ... --template <name>` so the CSV is tagged.")
        return
    available = list_templates(user_id)
    if filename_template not in available:
        say(f":warning: `{file_info['name']}` tags template `{filename_template}` which you don't have — skipping. "
            f"Your templates: {', '.join(f'`{t}`' for t in available)}")
        return
