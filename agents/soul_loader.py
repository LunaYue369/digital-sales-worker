...
def _get_user_soul(agent_id: str, user_id: str, template: str = "default") -> str | None:
    """Try to load a per-user soul from config/{dir}/{agent_id}s/.

    Tries {template}.md first, falls back to default.md.
    """
    if not user_id:
        return None
    user_dir = Path(user_config_dir(user_id)) / f"{agent_id}s"

    # Try template-specific soul first (e.g. combo.md, discovery.md)
    template_path = user_dir / f"{template}.md"
    if template_path.exists():
        return template_path.read_text(encoding="utf-8")

    # Fall back to the user's default soul
    default_path = user_dir / "default.md"
...
def build_system_prompt(agent_id: str, user_id: str = "", template: str = "default") -> str:
    ...
    3. config/{dir}/{agent_id}s/{template_or_default}.md
                                                    (if exists — per-user style/preferences)

    Per-user soul is ADDITIVE, not a replacement. This way per-user files
    only need to contain what's different (e.g. a sales rep's restaurant-only voice),
    while the generic agent soul provides the base (input format, output format, etc.).

    Template support: tries {template}.md first, falls back to default.md.
    """
    parts = []
    if _shared:
